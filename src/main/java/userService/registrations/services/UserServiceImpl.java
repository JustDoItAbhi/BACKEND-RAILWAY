...
//import userService.registrations.brevoemailservice.ResendEmailService;
...
//    private RedisTemplate redisTemplate;
    ...
    public String studentSignup(String email, String role) {
        ...
        if(roles.get().getRoleName().equals("STUDENT")){
            ...
            emailService.sendOtp("arvinderpalsingh2321@gmail.com","REQUEST FOR SIGN UP FROM TEACHER ",
                    "ALLOW TEACHER TO SIGN UP /n "+email+" THANK YOUR");
...
//        userRepository.save(users);
    ...
    public String confirmStudentOtp(String email, String otp) {
        System.out.println("REDIS CONFIRM OTP ");
        ...
        System.out.println("EMAIL EXSISTS "+email);
        String emailVlaue=otp+email;
...
//        redisTemplate.opsForValue().get("opt"+email);
    ...
    public UserResponseDto createUser(SignUpRequestDto dto) {
        ...
        if(users.getRolesList().equals("STUDENT")){
            emailService.sendOtp(users.getEmail(),"WELCOME TO S.C.P.M.S MANAGEMENT SYSTSM 😎", "WORK HARD AND ACHIEV YOUR GOALS , ALL THE BEST !!! 👍");
            System.out.println("EMAIL SENT FOR SIGNUP ");
        }
        if(users.getRolesList().equals("TEACHER")){
            emailService.sendOtp(users.getEmail(),"WELCOME TO S.C.P.M.S MANAGEMENT SYSTSM 😎", "thank you for joining us we will together make our city and university successfull ALL THE BEST !!! 👍");
            System.out.println("EMAIL SENT FOR SIGNUP ");
        }
...
//        redisTemplate.opsForValue().set(email,otp);
        ...
        emailService.sendOtp(users.getEmail(),"YOUR OTP IS "+ otp, "THANK YOUR");
